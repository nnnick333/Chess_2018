...
class Game():
        ...
        for m in range(len(board)):
            for n in board[m]:
                if n != None:
                    ...
                    if n.type == "king":
                        n.king(board)
    #Method checks for whether or not the king of one side is in the path of a piece from another side
                    ...
                    for (dx,dy) in x.moves:
                        ...
                        if n != None and n.type == "king" and n.side != x.side:
                            if x.side != game.player:
                                #If the king has any moves left(game ending moves are removed via 'remove_danger' method) then its check
                                ...
                                if self.moves_left(n.side,board) == 0:
                                    game.var_checkmate = n.side
                                    return ("Checkmate")

                        
    #Checks for whether or not a movement is legal(checkmating yourself is illegal)
                ...
                if x != None and x.side != side and len(x.moves) != 0:
                    for (dx,dy) in x.moves:
                        ...
                        if n != None and n.type == "king" and side != x.side:
                            return False
    #Method creates a theoretical board, simulating each move and removing moves that put the player into checkmate
            ...
            for n in self.board[m]:
                if n != None:
                    ...
                    for x in range(len(n.moves)):
                        ...
                        if n.side == 2:
                            move_counter2 += 1
        #Exception: If there are two kings left then it is stalemate
        if piece_counter == 2 or move_counter1 == 0 or move_counter2 == 0:
            return True
    #METHOD CALCULATES ALL MOVES AND SCENARIOS EACH TURN
    def Move_Calculations(self):
        ...
        self.calc_moves(game.board)
        #If a side is in check then it sets the game attribute 'game.var_check' to side
...
#DISPLAYS THE CHESSBOARD GRID(PARAMETERS ARE POSITION AND WIDTH OF EACH SQUARE)
...
def display_status():
    ...
    if game.status == "Checkmate":
        if game.var_checkmate == 1:
            text("White Wins",350,650)
        if game.var_checkmate == 2:
...
#IF THE SELECTED PIECE HAS AVAILABLE MOVES THEN ALLOW SELECTION
def piece_highlight(n):
...
#MOUSE SELECTION FOR PAWN UPGRADE PANEL
def pawn_upgrade():
    n = game.upgrade
    game.board[n.y][n.x] = None
    if 275 <= mouseX <= 315:
...
#COMBINES IN-GAME DISPLAY FUNCTIONS
def display_ingame():
    if game.status != None:
        display_status()
...
def display_scores():
    ...
    if len (game.players) >= 1:
        text(game.players_high[-1][1],150,200)
        text(game.players_high[-1][0],485,200)
    if len (game.players) >= 2:
        text(game.players_high[-2][1],150,300)
        text(game.players_high[-2][0],485,300)
    if len (game.players) >= 3:
        text(game.players_high[-3][1],150,400)
        text(game.players_high[-3][0],485,400)
    if len (game.players) >= 4:
        text(game.players_high[-4][1],150,500)
        text(game.players_high[-4][0],485,500)
...
def display_title_screen():
    ...
    textAlign(CENTER)
    text("CHESS",350,70)
    text("TOP PLAYERS", 350, 320)
    fill(255)
    strokeWeight(2)
    text("PLAY CHESS CLASSIC",350,200)
...
#TITLE SCREEN MOUSE SELECTION
def mouse_title_screen():
    if mouse_Boundaries(128,578,150,220):
        game.mode = 1
        starts()
    if mouse_Boundaries(128,578,270,340):
        game.mode = 2

#IN-GAME MOUSE SELECTION(PAWN UPGRADE AND PIECE SELECTION)
def mouse_ingame():
    if game.upgrade != None:
...
def fileGrab():
    ...
    for n in range(list_length):
        if n % 2 == 0:
            ...
            for m in range(len(temp_all[n])):
                if temp_all[n][m] == '':
                    break
            temp_names.append(temp_all[n][:m])
